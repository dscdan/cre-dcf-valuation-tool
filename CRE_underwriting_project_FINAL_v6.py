...
def calculate_deal_score(metrics, property_data):
    ...
    try:
        ...
        # Cap rate spread scoring (10 pts) - FIXED LOGIC
        # Positive spread means exit cap > going-in cap (compression = bad)
        # Negative spread means exit cap < going-in cap (expansion = good)
        cap_score = 0
        if cap_spread <= -1.0:     cap_score = 10  # Exit cap much lower (great appreciation)
        elif cap_spread <= -0.5:   cap_score = 9   # Exit cap lower (good appreciation)
        elif cap_spread <= 0:      cap_score = 7   # Exit cap same or slightly lower (neutral/OK)
        elif cap_spread <= 0.5:    cap_score = 5   # Exit cap slightly higher (slight compression)
        elif cap_spread <= 1.0:    cap_score = 3   # Exit cap higher (compression risk)
        else:                      cap_score = 1   # Exit cap much higher (major compression risk)
        
        # Entry valuation vs market (5 pts) - MORE REALISTIC
        property_type = property_data.get('property_type', '')
        market_caps = {'Multifamily': 5.0, 'Office': 7.0, 'Retail': 6.5, 'Single Family': 5.5}
        market_cap = market_caps.get(property_type, 6.0)
        
        # More balanced entry scoring
        cap_diff = going_in_cap - market_cap
        entry_score = 0
        if cap_diff >= 1.5:      entry_score = 5  # Buying 1.5%+ above market (value deal)
        elif cap_diff >= 0.75:   entry_score = 4  # Buying 0.75%+ above market (good)
        elif cap_diff >= 0:      entry_score = 3  # Buying at/above market (fair)
        elif cap_diff >= -0.75:  entry_score = 2  # Buying slightly below market (aggressive)
        else:                    entry_score = 1  # Buying well below market (very aggressive)
...
# Sidebar
with st.sidebar:
    st.header("📥 Import Deal")
    
    uploaded_file = st.file_uploader(
        "Upload Excel Deal File",
        type=['xlsx'],
        help="Import a previously exported deal Excel file to restore all inputs",
        key='deal_upload'
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            if len(import_df) > 0:
                ...
                # Load all inputs into session state
                st.session_state['property_name_input'] = row.get('Property Name', 'Imported Property')
                st.session_state['property_type_input'] = row.get('Property Type', 'Multifamily')
                
                # Size metric
                prop_type = row.get('Property Type', 'Multifamily')
                if prop_type == "Single Family":
                    st.session_state['size_metric_input'] = 'house'
                elif prop_type == "Multifamily":
                    st.session_state['size_metric_input'] = 'units'
                else:
                    st.session_state['size_metric_input'] = 'sf'
                
                st.session_state['units_or_sf_input'] = int(row.get('Units/SF', 10))
                st.session_state['purchase_price_input'] = float(row.get('Purchase Price', 1000000))
                st.session_state['closing_costs_input'] = float(row.get('Closing Costs', 20000))
                st.session_state['rent_growth_input'] = float(row.get('Rent Growth %', 3.0))
                st.session_state['vacancy_input'] = float(row.get('Vacancy %', 5.0))
                st.session_state['opex_growth_input'] = float(row.get('OpEx Growth %', 3.0))
                st.session_state['management_fee_input'] = float(row.get('Management Fee %', 4.0))
                st.session_state['loan_to_value_input'] = float(row.get('LTV %', 70.0))
                st.session_state['interest_rate_input'] = float(row.get('Interest Rate %', 6.5))
                st.session_state['amortization_input'] = int(row.get('Amortization (Years)', 30))
                st.session_state['io_period_input'] = int(row.get('IO Period (Years)', 0))
                st.session_state['holding_period_input'] = int(row.get('Holding Period (Years)', 10))
                st.session_state['exit_cap_rate_input'] = float(row.get('Exit Cap Rate %', 5.5))
                st.session_state['selling_costs_input'] = float(row.get('Selling Costs %', 2.0))
                st.session_state['discount_rate_input'] = float(row.get('Discount Rate %', 12.0))
                st.session_state['tax_rate_input'] = float(row.get('Tax Rate %', 0.0))
                
                # Property-specific fields
                if prop_type == "Single Family":
                    st.session_state['unit_0_rent_input'] = float(row.get('SF Rent', 1500))
                    st.session_state['opex_per_unit_or_sf_input'] = float(row.get('OpEx Total', 3000))
                    st.session_state['capex_per_unit_or_sf_input'] = float(row.get('CapEx Total', 300))
                elif prop_type == "Multifamily":
                    # Parse comma-separated rents
                    if 'Unit Rents' in row and pd.notna(row['Unit Rents']):
                        rents = [float(x.strip()) for x in str(row['Unit Rents']).split(',')]
                        for i, rent in enumerate(rents):
                            st.session_state[f'unit_{i}_rent_input'] = rent
                    st.session_state['opex_per_unit_or_sf_input'] = float(row.get('OpEx Per Unit', 5000))
                    st.session_state['capex_per_unit_or_sf_input'] = float(row.get('CapEx Per Unit', 300))
                else:  # Office or Retail
                    st.session_state['rent_per_sf_input'] = float(row.get('Rent Per SF', 25))
                    st.session_state['opex_per_unit_or_sf_input'] = float(row.get('OpEx Per SF', 8))
                    st.session_state['capex_per_unit_or_sf_input'] = float(row.get('CapEx Per SF', 0.5))
                
                st.success(f"✅ Imported: {row.get('Property Name', 'Property')}")
                st.rerun()
        except Exception as e:
...
save_col1, save_col2, save_col3, save_col4, save_col5 = st.columns([2, 1, 1, 1.2, 1.2])
...
with save_col3:
    st.write("")
    st.write("")
    # Excel Export - Complete deal data
    excel_data = {
        'Property Name': [property_name],
        'Property Type': [property_type],
        'Units/SF': [units_or_sf],
        ...
        'Amortization (Years)': [amortization],
        'IO Period (Years)': [io_period],
        'Holding Period (Years)': [holding_period],
    ...
    if property_type == "Single Family":
        excel_data['SF Rent'] = [unit_rents[0]]
        excel_data['OpEx Total'] = [opex_per_unit_or_sf]
        excel_data['CapEx Total'] = [capex_per_unit_or_sf]
    elif property_type == "Multifamily":
        # Store as comma-separated for multifamily
        excel_data['Unit Rents'] = [','.join(map(str, unit_rents))]
        excel_data['OpEx Per Unit'] = [opex_per_unit_or_sf]
        excel_data['CapEx Per Unit'] = [capex_per_unit_or_sf]
    else:
        excel_data['Rent Per SF'] = [rent_per_sf if property_type in ["Office", "Retail"] else 0]
        excel_data['OpEx Per SF'] = [opex_per_unit_or_sf]
        excel_data['CapEx Per SF'] = [capex_per_unit_or_sf]
    ...
    st.download_button(
        "📊 Export Excel",
        excel_buffer,
        f"{property_name.replace(' ', '_')}_deal_data.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        help="Download complete deal inputs - can be re-imported later!"
    )
...
with save_col4:
    ...
    st.download_button("📥 CSV", csv, f"{property_name.replace(' ', '_')}_cashflow.csv", 
...
with st.expander("ℹ️ How is the Investment Score Calculated?"):
    ...
    - **What it measures:** Cap rate trajectory and entry valuation vs market
    - **Calculation:**
        - **Cap Rate Spread (Exit - Entry):** 10 pts
            - Negative spread (cap compression) = Full points (property appreciating)
            - Zero spread (stable cap) = Good points (maintaining value)
            - Positive spread (cap expansion) = Low points (value erosion risk)
        - **Entry Cap vs Market Benchmark:** 5 pts
            - Buying above market cap rate = Full points (value purchase)
            - Buying at market cap rate = Good points (fair deal)
            - Buying below market cap rate = Low points (premium pricing)
